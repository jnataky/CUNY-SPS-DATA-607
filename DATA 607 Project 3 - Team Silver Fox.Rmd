...
Create the table of Answers with Q_ID, A_ID, Answer as columns. Later, we'll tie the A_ID and Q_ID together in a separate table but for now we'll use the Q_ID here to ensure we're maintaing the "tie between":
...
#First gather all associated answers into one column then cut out the columns we aren't interested in, then count
...
#Relabel column headers with corresponding Q_ID
